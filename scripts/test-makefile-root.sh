...
MARK="$TEMP_ROOT/make-syntax"; BAD="\$(shell /usr/bin/touch '$MARK')"; for variable in PYTHON XCODEBUILD; do if (cd "$CONTROL_DIR"&&/usr/bin/make --no-print-directory -f "$MAKEFILE" "$variable=$BAD" check) >/dev/null 2>&1; then exit 1; fi; [ ! -e "$MARK" ]; done
